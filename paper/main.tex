...
\begin{figure}[htbp]
  ...
  \caption[Scaling and ProteoBench accuracy]{%
    ...
    \texttt{executor.queueSize}) on the PXD071075 single-cell cohort, log--log axes.
    The dashed grey line is ideal linear ($1{/}N$) strong scaling anchored at the
    10-node point; parallel efficiency (measured/ideal speed-up) is printed under each
    dot. The measured curve bends above the ideal beyond \textasciitilde 100 nodes
    (efficiency 100\%~$\to$~58\% over the sweep), reflecting both the fixed serial
    cohort-level steps and shared-cluster resource contention at high concurrency on
    this EMBL-EBI allocation (see text); each point is a single run.
...
\begin{figure}[htbp]
  ...
  \caption[Reanalysis vs original analysis]{%
    ...
    per-precursor reports under the same global rule (Supplementary Note~3). The
