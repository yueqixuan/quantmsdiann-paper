...
Public DIA deposits are typically processed once, with the search engine available at the time of deposition, and are seldom revisited --- leaving recoverable identifications in raw data that has already been collected, paid for, and published. Reanalysing these deposits with a current \texttt{DIA-NN} build recovers them: across every deposit reanalysed here --- single-cell, bulk cell-line and spatial --- reprocessing yielded more protein groups than the originally deposited or published analysis, and more precursors wherever the original was itself \texttt{DIA-NN}, on every deposit (Fig.~\ref{fig:reanalysis}). The recovery is therefore not a property of any single dataset but a systematic return on reprocessing archived DIA data with an up-to-date engine. Three caveats bound how these cross-deposit comparisons should be read. First, \emph{FDR calibration}: the comparisons assume that recent \texttt{DIA-NN} (2.5/2.5.1) exercises largely correct false-discovery-rate (FDR) control, whereas older releases and other search engines may have substantially higher true FDR at the same nominal 1\% q-value~\cite{wen2025fdr,krull2024singlecell}, and verifying their FDR calibration on each deposited dataset is beyond the scope of this work; the reported gains should therefore be read as identifications recovered under a current, FDR-controlled engine, not as strictly FDR-matched differences. Second, \emph{counting criterion}: for the same reason we restrict cross-engine comparisons to identification counts under the original studies' own thresholds, and do not compare plexDIA protein groups or phosphoproteomic site numbers across engines (Methods). Third, \emph{sequence database and search settings}: deposited analyses may differ from the reanalysis in protein sequence database and search configuration, including contaminant databases that are frequently not deposited alongside the results (as for the spatial DVP cohort, PXD064049); because the reanalysis applies one harmonised FASTA and parameter set across all cohorts, residual count differences attributable to such undocumented database or setting choices cannot be fully excluded.
To show the value of running one SDRF-driven workflow across many independent deposits, we reanalysed five public cell-line DIA panels in a single \quantmsdiann{} invocation: NCI-60 (PXD003539, Guo \textit{et al.}~2019~\cite{guo2019nci60}; originally OpenSWATH), ProCan-DepMapSanger (PXD030304, Gon\c{c}alves \textit{et al.}~2022~\cite{goncalves2022procan}; originally \texttt{DIA-NN}~1.7.x), the Sun \textit{et al.}~2023 PCT-SWATH breast-cancer panel (PXD004701, 76 lines~\cite{sun2023breast}), the MultiPro batch-effect testbed (PXD041421, Wang \textit{et al.}~2023~\cite{wang2023multipro}), and Tognetti \textit{et al.}~2021 (PXD017199, 67 breast-cancer lines~\cite{tognetti2021breast}) (Supplementary Fig.~S7).
...
\begin{figure}[htbp]
  ...
  \caption[Scaling and ProteoBench accuracy]{%
    ...
    10 nodes; parallel efficiency is printed under each dot. The curve bends above
    ideal beyond \textasciitilde 100 nodes (efficiency 100\%~$\to$~58\%), reflecting
    fixed serial cohort-level steps and shared-cluster contention on this EMBL-EBI
    allocation.
