...
Because the \texttt{DIA-NN} license restricts redistribution, only version~1.8.1 is shipped as an image (the default, from BioContainers); releases from 1.9 onward (1.9.2--2.5.1, including the 2.5.1 enterprise build) are built locally from the per-release recipes in the companion \texttt{quantms-containers} repository (\url{https://github.com/bigbio/quantms-containers}) and tagged so the workflow selects them automatically (Additional file~1).
...
Additional file~1 provides Table~S1 (the full PRIDE accession list), per-process runtime and resource envelopes (Figs.~S1--S2), the ProteoBench per-version identification breakdown (Fig.~S3), and per-cohort completeness, peptide-per-protein and gene-versus-accession overlap figures (Figs.~S4--S12).
