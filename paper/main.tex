...
Identification depth, by contrast, rises monotonically with \texttt{DIA-NN} version on every module (precursors $+$14--23\%, per-run protein groups $+$8--14\%, and proteins quantified in every run $+$12--20\% from 1.8.1 to 2.5.1-enterprise; the academic 2.5.1 build and the full per-version, per-module breakdown are in Supplementary Note~5, Fig.~S3), in line with the broader ProteoBench community trend~\cite{devreese2025proteobench} and motivating reanalysis of public deposits with the current build (Sections~\ref{sec:single-cell}--\ref{sec:reanalyses}).
...
The newer 2.5.1-enterprise version increased total identifications on both cohorts: on HeLa Astral, union precursors rose from 19{,}365 to 23{,}712 ($+22$\%) and protein groups from 3{,}903 to 4{,}574 ($+17$\%); on One-Tip, precursors rose from 10{,}627 to 15{,}321 ($+44$\%) and protein groups from 1{,}597 to 2{,}306 ($+44$\%; Fig.~\ref{fig:single-cell}a), with the gain holding per cell (Fig.~\ref{fig:single-cell}b).
...
\begin{figure}[htbp]
  ...
  \caption[Pan-cohort of DIA reanalyses]{%
    ...
    (ProCan-DepMapSanger, 947 lines), PXD004701 (Sun 2023, 76 breast
