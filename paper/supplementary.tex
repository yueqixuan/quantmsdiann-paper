...
\begin{figure}[htbp]
  ...
  \caption[ProteoBench parity]{%
    \textbf{\quantmsdiann{} versus parameter-matched standalone
    \texttt{DIA-NN} GUI submissions.}
    Precursor and protein-group identification counts for \quantmsdiann{}'s
    five \texttt{DIA-NN} versions (blue) alongside the ProteoBench
    \texttt{DIA-NN} GUI submissions whose acquisition and search parameters
    most closely match (grey). \quantmsdiann{}'s counts fall within the range
    spanned by the parameter-matched GUI submissions, confirming that the
    SDRF-driven wrapper does not erode \texttt{DIA-NN} performance; the
    residual spread among the grey points reflects parameter and
    \texttt{DIA-NN}-version differences between community submissions.
  }
...
\begin{figure}[htbp]
  ...
  \caption[ProteoBench per-version per-module breakdown]{%
    ...
    and per module: a detailed view.}
