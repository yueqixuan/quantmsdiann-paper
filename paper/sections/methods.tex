...
\texttt{DIA-NN} is handled separately because its license restricts redistribution: only version~1.8.1 is publicly distributable and is pulled as the default from BioContainers (\texttt{docker.io/biocontainers/diann:v1.8.1\_cv1}), whereas \texttt{DIA-NN} releases from 1.9 onward (1.9.2, 2.0.2, 2.1.0, 2.2.0, 2.3.2, 2.5.0) cannot be redistributed and are not shipped as images.
For those versions the pipeline provides build recipes in the companion \texttt{quantms-containers} repository (\url{https://github.com/bigbio/quantms-containers}), one per release; a user with a valid \texttt{DIA-NN} download builds the container locally, for example \texttt{cd diann-2.5.0 \&\& docker build -t ghcr.io/bigbio/diann:2.5.0 .} (and, optionally, \texttt{singularity build diann-2.5.0.sif docker-daemon://ghcr.io/bigbio/diann:2.5.0}).
For the workflow to pick up a locally built image automatically, it must be tagged with the prefix \texttt{ghcr.io/bigbio/diann:} followed by the release tag (e.g.\ \texttt{ghcr.io/bigbio/diann:2.5.0}), which is the container assigned to every \texttt{withLabel:~diann} process; the desired release is then activated at runtime with \texttt{-profile diann\_v<version>} (equivalently the \texttt{--diann\_version} parameter), with \texttt{DIA-NN}~1.8.1 as the default.
